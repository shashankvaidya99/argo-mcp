...
def _validate_output(data: dict, raw_output: str) -> None:
    ...
    if the data does not match iso42001_output_schema.json — so a malformed SoA
    draft fails loudly instead of being returned to the caller.
    """
    schema = _load_schema()
    try:
