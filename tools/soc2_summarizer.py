...
def _validate_output(data: dict, raw_output: str) -> None:
    ...
    if the data does not match soc2_output_schema.json — so a malformed audit
    summary fails loudly instead of being returned to the caller.
    """
    schema = _load_schema()
    try:
